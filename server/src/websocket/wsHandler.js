  ...
  ws.on('message', async (raw) => {
    ...
    switch (msg.type) {
      ...
      case 'share': {
        ...
        console.log(`[WS] ── Share received from ${clientMeta.telegramId} ──`);
        console.log(`[WS]   jobId: ${shareData?.jobId}  meetsPool: ${shareData?.meetsPool}  realHash: ${shareData?.realHash}`);
        console.log(`[WS]   nonce2: ${shareData?.nonce2Hex?.slice(0,24)}... (${(shareData?.nonce2Hex?.length || 0) / 2} bytes)`);
        console.log(`[WS]   hash: ${shareData?.hash?.slice(0,24)}...`);
        console.log(`[WS]   stratum connected: ${stratum?.connected}  subscribers: ${stratum?.subscribers?.size}`);
        ...
        console.log(`[WS]   antiCheat: ${validation.valid ? 'PASS' : `FAIL (${validation.reason})`}`);

        if (validation.valid) {
          // Forward to real pool if Stratum is connected
          if (stratum?.connected) {
            const result = stratum.submitShare(ws, {
              jobId:      shareData?.jobId,
              nonce2Hex:  shareData?.nonce2Hex,
              time:       shareData?.time,
              hash:       shareData?.hash,
              solution:   shareData?.solution,   // full 2694-char solution from WASM miner
              meetsPool:  shareData?.meetsPool,
              realHash:   shareData?.realHash,
              minerId:    clientMeta.telegramId,
            });

            if (!result.forwarded) {
              console.log(`[WS]   ❌ NOT forwarded: ${result.reason}`);
            } else {
              console.log(`[WS]   ✅ Share forwarded to pool!`);
            }
          } else {
            console.warn(`[WS]   ❌ No stratum connection — share dropped`);
          }
        } else {
          console.warn(`[WS]   ❌ Invalid share: ${validation.reason}`);
        }
