...
const MAX_HASHRATE_MH = parseFloat(process.env.MAX_HASHRATE_MH || '100') * 1_000_000; // 100 MH/s (supports 16+ core multi-threaded mining)
const MAX_SHARES_PER_MINUTE = parseInt(process.env.MAX_SHARES_PER_MINUTE || '600');
 ...
 */
function isValidHash({ hash, difficulty = 3, meetsPool, meetsLocal }) {
  if (typeof hash !== 'string') return false;
  // If the worker already validated against pool target or local difficulty, trust it
  if (meetsPool || meetsLocal) return true;
  // Fallback: check leading zeros
